<?php
include "../koneksi.php";
$sql="select max(c1) as max_c1, max(c2) as max_c2, max(c3) as max_c3,max(c4) as max_c4,max(c5) as max_c5 from tbl_nilai";
...
while ($row=mysqli_fetch_array($hasil)) {
 ...
 $skor[$i]=round((($w1*$c1_normalisasi)+($w2*$c2_normalisasi)+($w3*$c3_normalisasi)+($w4*$c4_normalisasi)+($w5*$c5_normalisasi)),3);
 $i++;
}
